class Netstat {
    constructor(parentId) {
        ...
        this.offline = false;
        this.lastconn = {finished: true}; // Prevent geoip lookup attempt until maxminddb is loaded
        this.iface = null;
        this.failedAttempts = {};
    ...
    updateInfo() {
        window.si.networkInterfaces().then(async data => {
            ...
            if (net.ip4 === "127.0.0.1") {
                ...
                let p = await this.ping(window.settings.pingAddr || "baidu.com", 80, net.ip4).catch(() => { offline = true });
